...
use mfw_cmca_bridge::{
    project_allocation, select_projected_mass, CmcaAllocation8, CmcaBridgeRefusal, Q16_ONE,
};
...
fn main() {
    let allocation = CmcaAllocation8 {
        q16_16: [0, 16_384, 0, Q16_ONE, 0, 0, 0, 0],
    };
    ...
    assert_eq!(
        select_projected_mass(&projected)
            .expect("selection")
            .best_id,
        3
    );
    ...
    bad[7] = Q16_ONE + 1;
    assert!(matches!(
        project_allocation(&input(), &CmcaAllocation8 { q16_16: bad }),
        Err(CmcaBridgeRefusal::AllocationOutOfRange)
    ));

    let mut policy_refused = projected;
    policy_refused.policy_valid = false;
    assert_eq!(
        select_projected_mass(&policy_refused),
        Err(AutoSelectRefusal::ControlStateUnadmitted)
    );

    println!(
        "subject=mfw@a808821c6636535bc80f59f660b5b35906948c8e bridge_blob=64e2c55b574432525af68637f7b3fe843152f592 selected=3 mass1=64 mass3=255 empty_refused=1 range_refused=1 policy_refused=1 actuation=0"
    );
}
